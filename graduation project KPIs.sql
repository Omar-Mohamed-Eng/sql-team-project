...
SELECT 
  ORIGIN_AIRPORT,
  COUNT(ORIGIN_AIRPORT) AS flightss_from_origin
FROM flights
WHERE CANCELLED = 0
...
SELECT 
  DESTINATION_AIRPORT,
  COUNT( DESTINATION_AIRPORT) AS flightss_to_destination
FROM flights
WHERE CANCELLED = 0
...
-- 28- Avg.Delay & Taxi in for Destination airport 
